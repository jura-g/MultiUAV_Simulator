...
from graupner_serial.srv import BuildingDescriptor, GUITrajectoryParameters, LatLong
...
class RequestTrajectory():
	...
	self.service = rospy.Service('TrajectoryFromGUIService', GUITrajectoryParameters, self.trajectory_setup)
...
#	traj_wp_yaw = [self.position[self.selected_UAV][3]] + list(resp.traj_yaw)
	...
	traj_wp_x = req.desiredX
        traj_wp_y = req.desiredY
        traj_wp_z = req.desiredZ
        traj_wp_yaw = req.desiredW
